 ...
 */(file, record, search) => {
	...
	function createItem(itemData) {
		try {
			...
			if (itemData.price) {
				const basedPrice = itemData.price.substring(0, itemData.price.lastIndexOf(' '))
				itemRec.selectLine({
					sublistId: "price1",
					line: 0
				});
			...
			if (itemData.product_weight) {
				const weight = itemData.product_weight.substring(0, itemData.product_weight.lastIndexOf(' '))
				itemRec.setValue({
					fieldId: "weight",
					value: +weight,
				});
