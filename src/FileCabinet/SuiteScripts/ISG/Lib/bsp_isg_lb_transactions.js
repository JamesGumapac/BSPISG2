...
define([
  ...
  function createTransactionRecord(
    ...
    if (shipmentType === BSPLBUtils.SHIPMENT_TYPE.wrapAndLabel) {
      transactionRecord.setValue({
        fieldId: "custbody_bsp_isg_route_code",
        value: delZone.routeCode,
      });
      transactionRecord.setValue({
        fieldId: "location",
        value: delZone.location,
      });
    }
  ...
  function searchDeliveryZone(order) {
    ...
    customrecord_bsp_deliveryzoneSearchObj.run().each(function (result) {
      return {
        city: result.getValue({ name: "custrecord_bsp_city" }),
        state: result.getValue({ name: "custrecord_bsp_state_" }),
        zipCode: result.getValue({ name: "custrecord_bsp_zipcode" }),
        routeCode: result.getValue({ name: "custrecord_bsp_routecode" }),
        location: result.getValue({ name: "custrecord_bsp_location_" }),
      };
    });
